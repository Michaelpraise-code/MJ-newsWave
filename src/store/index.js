  ...
  actions: {
    ...
    async fetchHeadlineNews({ commit }) {
      const res = await axios.get(
        'https://alvahtek.com/projects/ota/api/news-api/news.php?type=headline',
      )
      commit('SET_HEADLINE_NEWS', res.data)
    },
    async fetchNewsByUuid({ commit }, uuid) {
      const res = await axios.get(
        `https://alvahtek.com/projects/ota/api/news-api/news.php?uuid=${uuid}`,
      )
      commit('SET_SELECTED_NEWS', res.data)
    },
    async fetchNewsByCategory({ commit }, category) {
      const res = await axios.get(
        `https://alvahtek.com/projects/ota/api/news-api/news.php?category=${category}`,
      )
